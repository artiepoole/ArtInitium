...
The Boot Info Structure is created by Stage 1b and passed to Stage 2 at address `0x7E00`. It provides essential system information gathered during the boot process.

## Structure Layout

| Offset | Size     | Field              | Description                                                          |
|--------|----------|--------------------|----------------------------------------------------------------------|
| 0x00   | 4 bytes  | `magic`            | Magic number "ARTI" (0x41525449) - Used to verify structure validity |
| 0x04   | 2 bytes  | `memory_map_count` | Number of E820 memory map entries (max 32)                           |
| 0x06   | 2 bytes  | `vbe_mode_count`   | Number of VBE modes queried (max 8)                                  |
| 0x08   | 4 bytes  | `memory_map_ptr`   | Pointer to memory map array (E820 entries)                           |
| 0x0C   | 4 bytes  | `vbe_info_ptr`     | Pointer to VBE controller info block (512 bytes)                     |
| 0x10   | 4 bytes  | `vbe_modes_ptr`    | Pointer to VBE mode info blocks array (256 bytes × 8 modes)          |
| 0x14   | 82 bytes | (padding)          | Reserved for future use                                              |

**Total Size:** 96 bytes

## Memory Map Entry Format (E820)

Each memory map entry is 24 bytes:

| Offset | Size    | Field           | Description                              |
|--------|---------|-----------------|------------------------------------------|
| 0x00   | 8 bytes | `base_address`  | Physical start address of memory region  |
| 0x08   | 8 bytes | `length`        | Size of memory region in bytes           |
| 0x10   | 4 bytes | `type`          | Memory type (1=usable, 2=reserved, etc.) |
| 0x14   | 4 bytes | `acpi_extended` | ACPI 3.0 extended attributes             |

## VBE Mode Info Block Format

Each VBE mode info block is 256 bytes and follows the VESA BIOS Extension specification.

## Queried VBE Modes

The bootloader queries the following VBE modes:
